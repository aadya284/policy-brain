...
import { QueryRequest, QueryResponse, ApiError } from '../types/index.js';
...
export class QueryController {
  ...
  async queryDocuments(req: Request, res: Response): Promise<void> {
    try {
      const { query, topK = 5 }: QueryRequest = req.body;
      ...
      const apiError: ApiError = {
        message: (error as any)?.message || 'Failed to process query',
        code: 'QUERY_ERROR',
      };
      ...
      const embeddingStatus = embeddingService.getModelInfo();
        ...
        res.status(200).json({
          ...
          services: {
            ...
            pinecone: {
              indexName: (pineconeStats as any).name || config.PINECONE_INDEX_NAME,
