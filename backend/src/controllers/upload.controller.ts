...
import { UploadResponse, ApiError } from '../types/index.js';
...
export class UploadController {
  ...
  async uploadDocument(req: Request, res: Response): Promise<void> {
    try {
      ...
      const chunks = await textProcessingService.processDocument(
        filePath,
        documentId,
        1000, // chunk size
        200  // overlap
      ...
      const apiError: ApiError = {
        message: (error as any)?.message || 'Failed to process document',
