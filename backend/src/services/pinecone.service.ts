...
import { DocumentChunk, EmbeddingResult, PineconeMatch } from '../types/index.js';
...
export class PineconeService {
      ...
      if (!indexExists) {
        ...
        await this.pinecone.createIndex({
          ...
          spec: {
            serverless: {
              cloud: 'aws',
              region: config.PINECONE_ENVIRONMENT as any,
            },
          },
        });
   ...
   */
  async storeEmbeddings(chunks: DocumentChunk[], embeddings: number[][]): Promise<void> {
    try {
   ...
   */
  async queryEmbeddings(queryEmbedding: number[], topK: number = 5): Promise<PineconeMatch[]> {
    try {
      const index = this.getIndex();
