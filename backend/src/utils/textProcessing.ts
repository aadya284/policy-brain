...
export class TextProcessingService {
  ...
  async extractTextFromPDF(filePath: string): Promise<string> {
    try {
      ...
      throw new Error(`Failed to extract text from PDF: ${(error as any)?.message || 'Unknown error'}`);
    }
  }
  ...
  async extractTextFromDOCX(filePath: string): Promise<string> {
    try {
      ...
      throw new Error(`Failed to extract text from DOCX: ${(error as any)?.message || 'Unknown error'}`);
    }
  }
  ...
  async extractTextFromTXT(filePath: string): Promise<string> {
    try {
      ...
      throw new Error(`Failed to read TXT file: ${(error as any)?.message || 'Unknown error'}`);
    }
  }
   ...
   */
  validateFile(filePath: string, maxSizeMB: number = 10): void {
