...
import { clientApi, QueryRequest, QueryResponse } from "@/lib/clientApi"
import "./globals.css"
...
export default function DashboardComponent({ onBackToHome }: { onBackToHome: () => void }) {
  ...
  const createMessageId = (prefix: string) => `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch documents
        const documents = await clientApi.getDocuments()
        ...
        const queries = await clientApi.getQueries()
        ...
        if (queries.length > 0) {
          ...
          const newMessages = recentQueries.flatMap((query, index) => [
            {
              id: createMessageId(`user-${query.id}-${index}`),
              type: "user" as const,
              content: query.query,
              timestamp: new Date(query.created_at ?? new Date().toISOString()),
            },
            {
              id: createMessageId(`assistant-${query.id}-${index}`),
  ...
  const uploadFileDirectly = useCallback(async (file: File, index: number, document_type: string) => {
    ...
    try {
      ...
      const result = await clientApi.uploadDocument(file, document_type);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await clientApi.processQuery(queryRequest);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) => updateFileDomain(index, value as Domain)}
                          ...
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) => updateFilePriority(index, value as Priority)}
                              disabled={uploadedFile.status === "processing"}
                            >
                              <SelectTrigger className="h-12">
                                <SelectValue />
